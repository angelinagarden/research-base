...
async function handleUpload(event) {
    try {
        ...
        // Генерируем настоящий presigned URL для S3
        const s3Params = {
            Bucket: S3_BUCKET,
            Key: key,
...
async function handleProcess(event) {
    ...
    try {
        // 1. Проверяем, есть ли уже обработанный текст в DynamoDB
        ...
        let extractedText = '';
        let textractStatus = 'cached';
        
        if (existingItem.Item) {
            console.log('Found cached text in DynamoDB');
            extractedText = existingItem.Item.extractedText;
        } else {
            console.log('No cached text found, processing with Textract...');
            
            // 2. Проверяем, существует ли файл в S3
            try {
            ...
            try {
                ...
                // 4. Сохраняем извлеченный текст в DynamoDB
                await dynamodb.put({
                    TableName: DYNAMODB_TABLE,
                    Item: {
                        fileId: fileId,
        ...
        // 5. Обрабатываем текст с помощью AI
        const extractedData = performProfessionalAIAnalysis(extractedText, fileId);
        
        // 6. Обновляем DynamoDB с результатами AI анализа
        await dynamodb.update({
            TableName: DYNAMODB_TABLE,
            Key: { fileId: fileId },
            UpdateExpression: 'SET aiAnalysis = :ai, processedAt = :processed',
        ...
        return {
            ...
            body: JSON.stringify({ 
                ...
                textLength: extractedText.length,
                rawText: extractedText.substring(0, 1000) // Первые 1000 символов для отладки
            })
        };
...
function performProfessionalAIAnalysis(text, fileId) {
    ...
    // Методология
    const methodology = {
        summary: 'Анализ документа для извлечения ключевых инсайтов',
