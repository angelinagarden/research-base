...
async function handleNotionCreate(event) {
    ...
    const properties = {
        ...
        "JournalOrSource": {
            rich_text: [{ text: { content: journal_or_source || "MISSING" } }]
        },
        "VolumePages": {
        ...
        "LanguageSource": {
            rich_text: [{ text: { content: language_source || "MISSING" } }]
        },
        "RegionOrContext": {
            rich_text: [{ text: { content: region_or_context || "MISSING" } }]
        },
        "CentralInsight": {
        ...
        "TheoreticalFramework": {
            rich_text: [{ text: { content: truncateText(Array.isArray(theoretical_framework) ? theoretical_framework.join('\n') : theoretical_framework || "MISSING") } }]
        },
        "Methodology": {
            rich_text: [{ text: { content: truncateText(methodology ? methodology.summary : "MISSING") } }]
        },
        "KeyFindings": {
            rich_text: [{ text: { content: truncateText(Array.isArray(key_findings) ? key_findings.join('\n') : key_findings || "MISSING") } }]
        },
        "Quote": {
        ...
        "OnWebsite": {
            checkbox: on_website || false
        },
        "ProvenanceNotes": {
            rich_text: [{ text: { content: provenance && provenance.notes ? truncateText(provenance.notes) : "MISSING" } }]
        },
        "FileUrl": {
            url: fileUrl || null
        }
...
async function handleUpload(event) {
    ...
    return {
        ...
        body: JSON.stringify({ 
            success: true, 
            fileId, 
            uploadUrl: `https://mock-s3.amazonaws.com/uploads/${fileId}.pdf`,
